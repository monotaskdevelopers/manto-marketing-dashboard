...
| Klaviyo campaign detail retry limits | `src/lib/integrations/klaviyo-sync.ts` | Keeps hourly/manual sync bounded while the current campaign-only ingestion slice fetches campaign tags and campaign audiences. | Move broader Klaviyo datasets into explicit queued/operator jobs instead of adding them to this sync. |
...
| Static review after Klaviyo campaign-only sync narrowing | Completed | Reviewed the campaign-only sync path, removed `page[size]` from campaign tag/audience relationship endpoints, reduced duplicate per-campaign detail requests, and did not run lint/typecheck/build/live sync because the current project instruction forbids build/test commands unless explicitly requested. |

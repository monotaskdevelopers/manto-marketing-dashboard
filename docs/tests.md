...
| Klaviyo campaign relationship and performance retry limits | `src/lib/integrations/klaviyo-sync.ts` and `src/lib/sync/run-sync.ts` | Keeps hourly/manual sync bounded while the current campaign-only ingestion slice fetches included campaign tags, the beta campaign-audience relationship map, and a capped set of paced campaign values report dates per run. | Move broader Klaviyo datasets and force-refresh historical report ranges into explicit queued/operator jobs instead of adding them to this sync. |
| Klaviyo incremental campaign metadata and performance planner | `src/lib/sync/run-sync.ts`, `src/lib/integrations/klaviyo-sync.ts`, and `supabase/migrations/S007-klaviyo-incremental-sync-state.sql` | Uses local campaign metadata and `klaviyo_sync_date_coverage` to skip unchanged campaigns and covered stable report dates before Klaviyo requests are sent, while refreshing recent mutable dates and targeted older changed campaigns. | Keep; verify with live sync logs before production launch. |
...
- Verify a repeated 30-90 day cron sync logs the campaign metadata `updatedSince` cursor, reads `klaviyo_sync_date_coverage`, skips covered stable dates, preserves the latest 3 mutable dates, caps selected full-day report dates, reports `deferredMissing`, and still adds targeted older changed campaigns.
- Verify a repeated manual sync refreshes the latest 7 report dates in the requested window, caps missing full-day catch-up work, and still skips older covered stable dates.
...
| Static review after Klaviyo incremental request planner | Completed | Reviewed migration, sync planner, Klaviyo request filters, and docs; ran `git diff --check`. Did not run lint, typecheck, build, or live sync because the current project instruction forbids build/test commands unless explicitly requested. |
| Static review after Klaviyo bounded report catch-up fix | Completed | Fixed the beta campaign-audience relationship map to avoid the invalid `messages.channel` filter, capped full-day campaign report catch-up dates per run, updated docs, and ran `git diff --check`. Did not run lint, typecheck, build, or live sync because the current project instruction forbids build/test commands unless explicitly requested. |

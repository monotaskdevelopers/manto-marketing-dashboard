...
The active Klaviyo sync is intentionally narrow. For now, it should fetch only campaign metadata needed by
the Campaigns table:
...
- Campaign metadata rows use `send_at`, then `scheduled_at`, then Klaviyo update/create dates as their local
  date dimension when campaign report rows are not present.
- The Campaigns page still respects the shared `preset`, `start`, and `end` URL filters.
- Campaign performance report rows can still be displayed if historical `klaviyo_campaign_reports` rows
  exist, but the current Klaviyo sync does not fetch Reporting API rows.

...
| `CampaignTable` | `/src/components/campaign-table.tsx` | Client-side Campaigns workspace section with filter-aware metric cards, currency-aware revenue benchmarks, instant in-memory search, region/audience/channel/status/tag filters, sortable columns, and message-type tooltips without database requests for table-only changes. |

...
| `MultiSelectDropdown` | `/src/components/multi-select-dropdown.tsx` | Compact custom checkbox dropdown for multi-value analytics filters with active chips, outside-click dismissal, Escape support, and micro-animations. |
...
| `CampaignTable` | `/src/components/campaign-table.tsx` | Client-side Campaigns workspace section with filter-aware metric cards, currency-aware revenue benchmarks, instant in-memory search, multi-select region/audience/channel/status/tag filters, sortable columns, filtered pagination, and message-type tooltips without database requests for table-only changes. |

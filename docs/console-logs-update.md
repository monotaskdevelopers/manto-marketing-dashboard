...
| --- | --- | --- | --- |
| `src/lib/sync/run-sync.ts` | Logs sync start, Shopify/Klaviyo per-region progress, campaign performance date-planning counts, per-table batched Supabase upsert attempts, zero-row skips, upsert success, sanitized database errors, and sync completion status. | No | Keep only if production observability is needed; otherwise replace with structured logger. |
...
| `src/lib/integrations/klaviyo-sync.ts` | Logs Klaviyo sync stage starts, endpoint paths without secrets, API revision, HTTP method/status, endpoint page counts, reduced-request fallbacks, campaign values report progress, optional endpoint skips, bounded 429 retries, produced row counts, and sanitized JSON:API error summaries by region slug. | No | Keep warnings/errors and consider moving count summaries to structured sync telemetry. |
